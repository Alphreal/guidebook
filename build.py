...
function schShow(n){{if(n>=SCHT.length){{schHide();return}}SCH.step=n;SCH.open=1;schClear();var sel=SCHT[SCH.step];var o=document.querySelector("[data-schov]");if(o){{if(o.parentNode!==document.body)document.body.appendChild(o);o.hidden=false}}var el=document.querySelector(sel);if(el){{el.classList.add("schlight");try{{el.scrollIntoView({{behavior:"smooth",block:"center"}})}}catch(e){{el.scrollIntoView()}}}}}}
...
document.addEventListener("click",function(e){{
 ...
 if(so){{schShow(SCH.step+1);return}}
