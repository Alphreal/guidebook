...
var CHS=[["chapter-1-self-regulation-time","Time","Plan & focus blocks"],["chapter-2-distraction-concentration","Distraction","One tab, phone away"],["chapter-3-interaction-support","Interaction","Ask with the template"],["chapter-4-fatigue-technology","Fatigue","One stream, move & rest"]];
var TG={{step:0,ch:0}};
...
var SCHT=["[data-user]",'[data-exp="saveuser"]',"#schedule .daypick"];
...
function schHide(){{SCH.open=0;schClear()}}
function schShow(n){{if(n>=SCHT.length){{schHide();return}}SCH.step=n;SCH.open=1;schClear();var sel=SCHT[SCH.step];var el=document.querySelector(sel);if(el){{el.classList.add("schlight");try{{el.scrollIntoView({{behavior:"smooth",block:"center"}})}}catch(e){{el.scrollIntoView()}}}}}}
...
function paint(){{
 ...
 document.querySelectorAll("[data-chcard]").forEach(function(card){{
  var sid=card.getAttribute("data-chcard");
  var bs=card.querySelectorAll("[data-check]");var dn=0;
  bs.forEach(function(el){{if(R[el.getAttribute("data-check")])dn++}});
  var msg=card.querySelector('[data-chmsg="'+sid+'"]');
  var btn=card.querySelector('[data-chdone="'+sid+'"]');
  if(msg)msg.textContent=dn+"/"+bs.length+" done";
  if(btn)btn.disabled=dn<bs.length;
 }});
 var mon=monday();
...
document.addEventListener("input",function(e){{
 ...
 if(el.hasAttribute&&el.hasAttribute("data-note")){{N[el.getAttribute("data-note")]=el.value;try{{localStorage.setItem(kk("gb-notes-v3"),JSON.stringify(N))}}catch(err){{}}}}
  if(el.hasAttribute&&el.hasAttribute("data-user")){{if(SCH.open&&SCH.step===0)schShow(1);return}}
}});
document.addEventListener("click",function(e){{
 ...
 var db=e.target.closest?e.target.closest("[data-day]"):null;
  if(db){{var di=db.getAttribute("data-day");if(D[di])delete D[di];else D[di]=1;save();paint();if(SCH.open)schHide();return}}
 var da=e.target.closest?e.target.closest("[data-dayact]"):null;
 if(da){{var a=da.getAttribute("data-dayact");if(a==="all")D={{"0":1,"1":1,"2":1,"3":1,"4":1,"5":1,"6":1}};else if(a==="weekdays")D={{"0":1,"1":1,"2":1,"3":1,"4":1}};else D={{}};save();paint();if(SCH.open)schHide();return}}
 var st=e.target.closest?e.target.closest("[data-schstart]"):null;
 if(st){{try{{if(!sessionStorage.getItem("gbSchAuto")&&(!hasK(kk("gb-days-v3"))||!onlineCount())){{sessionStorage.setItem("gbSchAuto","1");e.preventDefault();schShow(0);return}}}}catch(e3){{}}}}
 ...
 if(ex){{var k4=ex.getAttribute("data-exp");var out=document.querySelector("[data-expout]");
  if(k4==="saveuser"){{var inp=document.querySelector("[data-user]");user=inp&&inp.value?inp.value.trim():"";try{{localStorage.setItem(UKEY,user)}}catch(err){{}}R=loadJ(kk("gb-routines-v3"),{{}});W=loadJ(kk("gb-week-v3"),{{}});N=loadJ(kk("gb-notes-v3"),{{}});if(hasK(kk("gb-days-v3")))D=loadJ(kk("gb-days-v3"),{{}});else D={{"0":1,"1":1,"2":1,"3":1,"4":1}};save();paint();if(SCH.open)schShow(2);return}}
 ...
 var lk=e.target.closest?e.target.closest("[data-secretlock]"):null;
 if(lk){{SCH.lk=(SCH.lk||0)+1;if(SCH.lkt)clearTimeout(SCH.lkt);SCH.lkt=setTimeout(function(){{SCH.lk=0}},2500);if(SCH.lk>=5){{SCH.lk=0;var L=document.querySelector("[data-sealight]");if(L){{if(L.parentNode!==document.body)document.body.appendChild(L);L.hidden=false}}return}}}}
...
SVG_FLOW_CHANNEL = """<svg viewBox="0 0 400 250" role="img" aria-label="Three zones: anxiety, flow, boredom"><rect x="8" y="8" width="122" height="234" rx="12" fill="#1e3a5f"/><polyline points="20,66 34,52 48,66 62,52 76,66 90,52 104,66 118,52" fill="none" stroke="#e0f7fa" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><polyline points="20,86 34,72 48,86 62,72 76,86 90,72 104,86 118,72" fill="none" stroke="#7dd3fc" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" opacity=".7"/><circle cx="69" cy="122" r="11" fill="#fcd9b8" stroke="#1a1a1a" stroke-width="2"/><line x1="62" y1="132" x2="46" y2="110" stroke="#fcd9b8" stroke-width="5" stroke-linecap="round"/><line x1="76" y1="132" x2="92" y2="110" stroke="#fcd9b8" stroke-width="5" stroke-linecap="round"/><text x="69" y="172" text-anchor="middle" font-size="14" font-weight="bold" fill="#fff">TOO HARD</text><text x="69" y="192" text-anchor="middle" font-size="12" fill="#fda4af">anxiety</text><text x="69" y="212" text-anchor="middle" font-size="11" fill="#94a3b8">drowning in</text><text x="69" y="226" text-anchor="middle" font-size="11" fill="#94a3b8">difficulty</text><rect x="139" y="8" width="122" height="234" rx="12" fill="#0e7490"/><circle cx="238" cy="38" r="12" fill="#fde68a"/><path d="M150 118 Q172 74 214 76 Q248 78 244 108" fill="none" stroke="#a5f3fc" stroke-width="5" stroke-linecap="round"/><circle cx="244" cy="112" r="4" fill="#a5f3fc"/><circle cx="252" cy="118" r="3" fill="#a5f3fc"/><line x1="182" y1="112" x2="226" y2="96" stroke="#1a1a1a" stroke-width="5" stroke-linecap="round"/><circle cx="206" cy="80" r="8" fill="#fcd9b8" stroke="#1a1a1a" stroke-width="2"/><line x1="206" y1="88" x2="204" y2="102" stroke="#fcd9b8" stroke-width="5" stroke-linecap="round"/><text x="200" y="172" text-anchor="middle" font-size="15" font-weight="bold" fill="#fff">FLOW</text><text x="200" y="192" text-anchor="middle" font-size="12" fill="#a5f3fc">ride it!</text><text x="200" y="212" text-anchor="middle" font-size="11" fill="#a5f3fc">challenge = skill</text><text x="200" y="226" text-anchor="middle" font-size="11" fill="#a5f3fc">protect this wave</text><rect x="270" y="8" width="122" height="234" rx="12" fill="#e0f7fa"/><rect x="270" y="150" width="122" height="92" rx="0" fill="#fde68a"/><line x1="282" y1="132" x2="380" y2="132" stroke="#0e7490" stroke-width="3" stroke-linecap="round"/><circle cx="326" cy="118" r="9" fill="#fcd9b8" stroke="#1a1a1a" stroke-width="2"/><line x1="326" y1="127" x2="326" y2="150" stroke="#fcd9b8" stroke-width="5" stroke-linecap="round"/><line x1="326" y1="150" x2="348" y2="150" stroke="#fcd9b8" stroke-width="5" stroke-linecap="round"/><circle cx="366" cy="52" r="14" fill="#fbbf24" opacity=".85"/><text x="331" y="192" text-anchor="middle" font-size="14" font-weight="bold" fill="#92400e">TOO EASY</text><text x="331" y="212" text-anchor="middle" font-size="12" fill="#92400e">boredom</text><text x="331" y="228" text-anchor="middle" font-size="11" fill="#a16207">flat calm water</text></svg>"""
...
def hub_grids(chaps, pages):
    ...
    def drop(c):
        h2 = re.sub(r'\s+id="[^"]+"', "", c["h2"], count=1)
        return (f'<details class="chapter" id="{c["sid"]}"><summary>{h2}</summary>'
...
CHECKLISTS = {
    "chapter-1-self-regulation-time": [
        ...
        "Finished 2 focus blocks (25-50 min each)",
    ],
    ...
    "chapter-4-fatigue-technology": [
        ...
        "Rested eyes at 2 natural breaks",
...
HOW = {
    ...
    "doc1-chapter-1-self-regulation-time-2": {
        "how": ["Set a 25-min timer to start — a minimum, not a stop sign.", "In flow (time disappears)? Cancel the alarm, keep going up to 50 min.", "Focus fading? Stop at a seam (end of section), break 5-15 min."],
        "ex": "25-min starter + one 45-min flow block, breaks between.",
        "self": ["Broke only between blocks?", "Extended one good flow?"],
    },
    ...
    "doc1-chapter-4-fatigue-technology-0": {
        "how": ["One stream only: lesson, nothing parallel (no feeds, no lyrics).", "Study 50 min.", "Move 10 min (stand, walk, stretch)."],
        "ex": "50 study - 10 walk, lesson tab only.",
        "self": ["Moved?", "One stream only?"],
    },
    "doc1-chapter-4-fatigue-technology-1": {
        "how": ["Rest eyes at natural breaks: end of a video or chapter.", "Look ~6m away ~20 seconds. Blink + roll shoulders.", "No clock-watching mid-lesson — seams, not signals."],
        "ex": "Rested eyes twice at breakpoints.",
        "self": ["At breakpoints, not mid-lesson?", "Twice today?"],
    },
...
def checklist_html(doc, sid, items):
    ...
    return (f'<div class="card" data-chcard="{sid}"><b>Try it this week - tick when done</b>'
            f'<div class="muted">Tap How? to see steps + self-check.</div>{"".join(parts)}'
            f'<div style="margin-top:10px"><button type="button" class="mini" data-chdone="{sid}">Done — send to teacher</button> '
            f'<span class="muted" data-chmsg="{sid}"></span></div></div>')
...
def secret_html():
    ...
    vault = ('<div class="sealight" data-sealight hidden><div class="seabox" role="dialog" aria-label="The Flow Stage secret method">'
             ...
             f'<figure class="seafig">{SVG_FLOW_CHANNEL}<figcaption>Find your wave: too hard drowns you, too easy bores you — flow is the rideable middle.</figcaption></figure>'
...
def main():
    ...
    idx_body = ('<div class="hero card"><div class="eyebrow">Asia University · Students</div>'
                ...
                f'<p><a href="{spage}" data-schstart>Start →</a></p></div>'
                + schedule_html() + "</div>"
