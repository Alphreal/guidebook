...
function schHide(){{SCH.open=0;if(SCH.tm)clearTimeout(SCH.tm);schClear();var o=document.querySelector("[data-schov]");if(o)o.hidden=true;var tp=document.querySelector("[data-schtip]");if(tp)tp.hidden=true}}
function schShow(n){{SCH.step=(n+SCHT.length)%SCHT.length;SCH.open=1;schClear();var s=SCHT[SCH.step];var o=document.querySelector("[data-schov]");if(o){{if(o.parentNode!==document.body)document.body.appendChild(o);o.hidden=false}}var tp0=document.querySelector("[data-schtip]");if(tp0){{if(tp0.parentNode!==document.body)document.body.appendChild(tp0);tp0.hidden=true}}var el=document.querySelector(s[0]);if(el){{el.classList.add("schlight");try{{el.scrollIntoView({{behavior:"smooth",block:"center"}})}}catch(e){{el.scrollIntoView()}}}}if(SCH.tm)clearTimeout(SCH.tm);SCH.tm=setTimeout(function(){{if(!SCH.open)return;var t2=document.querySelector("[data-schtip]");if(!t2)return;var th=t2.offsetHeight||220,sp=12;t2.style.left="50%";t2.style.transform="translateX(-50%)";var e2=document.querySelector(SCHT[SCH.step][0]);if(e2){{var r=e2.getBoundingClientRect();if(r.bottom+th+sp<window.innerHeight){{t2.style.top=(r.bottom+sp)+"px";t2.style.bottom="auto"}}else if(r.top-th-sp>0){{t2.style.top=Math.max(8,r.top-th-sp)+"px";t2.style.bottom="auto"}}else if(r.top>window.innerHeight*0.5){{t2.style.top="18px";t2.style.bottom="auto"}}else{{t2.style.top="auto";t2.style.bottom="18px"}}}}else{{t2.style.top="auto";t2.style.bottom="18px"}}t2.hidden=false;var h=t2.querySelector("[data-schtitle]");if(h)h.textContent=(SCH.step+1)+" / "+SCHT.length+" — "+SCHT[SCH.step][1];var x=t2.querySelector("[data-schtext]");if(x)x.textContent=SCHT[SCH.step][2];var bk=t2.querySelector('[data-sch="back"]');if(bk)bk.disabled=SCH.step===0;var nx=t2.querySelector('[data-sch="next"]');if(nx)nx.textContent=SCH.step===SCHT.length-1?"Done ✓":"Next ›"}},380)}}
